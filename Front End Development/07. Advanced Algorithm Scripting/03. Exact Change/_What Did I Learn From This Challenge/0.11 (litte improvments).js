...
function checkCashRegister(price, cash, cid) {
  ...
  function changing(inputChange) {
    var stillProgressing = foundValue.reduce(function(a, b) {
      if (summedCashInDrawer < change) {
        ...
        console.log(foundValue[a].name);
        console.log(foundValue[a].quantity);
        if (workingWithChange > foundValue[a].inDollars) {
          console.log('Wolumen banknotu:', foundValue[a].inDollars);
          var divisionOutcome = Math.floor(workingWithChange / foundValue[a].inDollars);
          var remainder = (workingWithChange / foundValue[a].inDollars - divisionOutcome) * foundValue[a].inDollars;
  ...
  return changeDone; //Elminster White - For now. I must change it.
}
