/*
Working example: https://repl.it/Efdn/0
  ...
  let result = arr.reduce(function(acc, next, index) {
    ...
    for(let i = 0; i <arr.length; i++) {
      if ((index !== index + i) && ((arr[index] + arr[index+i]) === arg)) {
        ...
        console.log("Found!", 'Number:', arr[index], 'Its index:', index, '---', 'Number:', arr[index+i], 'Its index:', index + i);
        console.log(index, index + i, '... and the sum of that two is:', index + index + i);
        ...
        for(let j = 0; j<arr.length; j++) {
          ...
          if (arr[j] === arr[index]) {
            console.log('Change value of arr[index]:', j);
            arr[j] = NaN;
          }
