...
function makeFriendlyDates(arr) {
  ...
  months = arr.map(function(oneMonthEl){
    //
    return monthNames[parseInt(oneMonthEl.substr(5, 2), 10) - 1]; // -1 because in monthNames "January" is 0
  });
  console.log(months);
  
  days = arr.map(function(oneDayEl){
    //
    return getOrdinal(parseInt(oneDayEl.substr(8, 2), 10));
  });
  console.log(days);
  ...
  // Set the two dates
  let first = new Date(2016, 7, 1);
  let second = new Date(2016, 7, 31);
