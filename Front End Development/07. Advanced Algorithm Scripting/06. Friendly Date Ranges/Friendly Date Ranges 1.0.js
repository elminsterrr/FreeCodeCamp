/*
Working example: https://repl.it/Ebab/0
...
function makeFriendlyDates(arr) {
  ...
  // Here program decides how to display results
  if (daysBetween(first, second) === 0) {
    return [monthNames[months[0]] + ' ' + getOrdinal(days[0]) + ', ' + years[0]];
  } else if (daysBetween(first, second) <= 30) {
    return [monthNames[months[0]] + ' ' + getOrdinal(days[0]), getOrdinal(days[1])];
  } else if ((daysBetween(first, second) < 365) && (new Date().getFullYear() === years[0])) {
    return [monthNames[months[0]] + ' ' + getOrdinal(days[0]), monthNames[months[1]] + ' ' + getOrdinal(days[1])];
  } else if (daysBetween(first, second) < 365) {
    return [monthNames[months[0]] + ' ' + getOrdinal(days[0]) + ', ' + years[0], monthNames[months[1]] + ' ' + getOrdinal(days[1])];
  } else {
    return [monthNames[months[0]] + ' ' + getOrdinal(days[0]) + ', ' + years[0], monthNames[months[1]] + ' ' + getOrdinal(days[1]) + ', ' + years[1]];
  }
}
