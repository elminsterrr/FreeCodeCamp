/*
Working example: https://repl.it/E8i2/0
...
function permAlone(firstInput) {
  ...
  permAloneOld(firstInput).map(function(oneReadyEl) {
    let letsCheckIt = 0;
    console.log(oneReadyEl);
    for (let i = 0; i<oneReadyEl.length; i++) {
      console.log(oneReadyEl[i]);
      // If oneReadyEl[i] for ex. 'a'
      // is the same as oneReadyEl[i -1] for ex. 'a'
      // we return false.
    ...
    // Only in case that our string doesn't
    // have repeated consecutive letters
    // we push oneReadyEl to found array.
