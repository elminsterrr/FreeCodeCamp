...
function smallestCommons(startArray) {
  var endArray = findAllNumbers(startArray);
  return magicFunction(endArray);
}
...
function magicFunction(inputEndArray) {
 // Variables needed declared outside the loops.
  ...
  do {
    quot = inputEndArray[0] * loop * inputEndArray[1];
    for (n = 2; n < inputEndArray.length; n++) {
      if (quot % inputEndArray[n] !== 0) {
        break;
      }
    }

    loop++;
  } while (n !== inputEndArray.length);
